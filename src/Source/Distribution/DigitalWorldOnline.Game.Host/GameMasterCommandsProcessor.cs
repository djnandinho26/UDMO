        ...
        public async Task ExecuteCommand(GameClient client, string message)
        {
            ...
            switch (command[0].ToLower())
            {
                case "hatch":
                {
                    ...
                    // Get evolution info
                    EvolutionAssetModel digimonEvolutionInfo =
                    ...
                    else
                    {
                        ...
                        {
                            var encyclopedia = CharacterEncyclopediaModel.Create(client.TamerId,
                                digimonEvolutionInfo.Id, newDigimon.Level, newDigimon.Size, 0, 0, 0, 0, 0, false,
                                false);
                            ...
                            _logger.Information(
                                $"Encyclopedia char id: {encyclopedia.CharacterId}, evos count: {encyclopedia.Evolutions.Count}");
                ...
                case "ban":
                {
                    ...
                    if (!string.IsNullOrEmpty(targetName))
                    {
                        ...
                        try
                        {
                            ...
                            var newBan = AccountBlockModel.Create((int)TargetBan.AccountId, banReason,
                                AccountBlockEnum.Permannent, DateTime.Now.AddHours(TimeBan));
                            ...
                            try
                            {
                                ...
                                if (isBanned != null)
                                {
                                    ...
                                    if (TargetTamer != null)
                                    {
                                        ...
                                        {
                                            var member =
                                                party.Members.FirstOrDefault(x => x.Value.Id == TargetTamer.TamerId);
                                            ...
                                            foreach (var target in party.Members.Values)
                                            {
                                                ...
                                                if (target.Id != client.Tamer.Id)
                                                    targetClient.Send(
                                                        new PartyMemberWarpGatePacket(party[client.TamerId])
                                                            .Serialize());
                                            }
                                        }
                                    }

        ...
        public async Task ExecuteCommand(GameClient client, string message)
        {
            ...
            switch (command[0].ToLower())
            {
                case "hatch":
                {
                    ...
                    _mapServer.BroadcastForUniqueTamer(client.TamerId,
                        new NeonMessagePacket(NeonMessageTypeEnum.Scale, client.Tamer.Name, newDigimon.BaseType,
                    ...
                    if (map != null)
                    {
                        ...
                        if (party != null)
                        {
                            ...
                            party.Members.Values.Where(x => x.Id != client.TamerId).ToList()
                                .ForEach(member =>
                                {
                                    _dungeonServer.BroadcastForUniqueTamer(member.Id,
                                        new PartyMemberWarpGatePacket(party[client.TamerId], member).Serialize());
                                });
